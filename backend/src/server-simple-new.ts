...
const pushToShopify = async (product: any, price: number, breakdown: any) => {
    try {
        ...
        try {
            ...
            const customMetafield = verifyResponse.data.metafields.find(
                (m: any) => m.namespace === 'custom' && m.key === 'code_form'
            );
            const geminiMetafield = verifyResponse.data.metafields.find(
